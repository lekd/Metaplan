...
namespace PostIt_Prototype_1.PostItDataHandlers
{
    ...
    public class GenericPostItCommandDecoder:IPostItNetworkDataHandler
    {
        ...
        {
            PostItCommandType commandType = classifyCommand(data);
            PostItCommand command = null;
            switch (commandType)
            {
                case PostItCommandType.Add:
                    command = decodeAddCommand(data);
                    break;
                case PostItCommandType.Update:
                    command = decodeUpdateCommand(data);
                    break;
                case PostItCommandType.Delete:
                    command = decodeDeleteCommand(data);
                    break;
            }
            if (command != null)
            {
        ...
        PostItCommand decodeAddCommand(byte[] data)
        {
            ...
            //X
            Array.Copy(data, index, buffer, 0, 4);
            note.Left = Utilities.UtilitiesLib.Bytes2Float(buffer);
            index += 4;
            //Y
            Array.Copy(data, index, buffer, 0, 4);
            note.Top = Utilities.UtilitiesLib.Bytes2Float(buffer);
            index += 4;
            ...
            note.Content = PostItNote.ParseContentFromBytes(note.DataType, buffer);
